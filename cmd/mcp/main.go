...
type mcpConfig struct {
	wordlistDir          string
	canonicalWordlistDir string // Pre-computed absolute symlink target
	scopeDir             string
	outputDir            string
	canonicalOutputDir   string // Pre-computed absolute symlink target
	auditLogPath         string
	maxThreads           int
	maxResults           int
...
func (r *scanRegistry) cleanupLocked() {
	now := time.Now()
	for id, scan := range r.scans {
		...
		if !running && !finishedAt.IsZero() && now.Sub(finishedAt) > 1*time.Hour {
			delete(r.scans, id)
		}
	}
}
...
func loadConfig(exePath string) (mcpConfig, error) {
	...
	// Pre-resolve dynamic allowed directories to avoid redundant EvalSymlinks filesystem calls
	absWL, err := filepath.Abs(cfg.wordlistDir)
	if err != nil {
		return mcpConfig{}, fmt.Errorf("canonical wordlist path: %w", err)
	}
	cfg.canonicalWordlistDir, err = filepath.EvalSymlinks(absWL)
	if err != nil {
		return mcpConfig{}, fmt.Errorf("canonical wordlist path symlink: %w", err)
	}

	absOut, err := filepath.Abs(cfg.outputDir)
	if err != nil {
		return mcpConfig{}, fmt.Errorf("canonical output path: %w", err)
	}
	cfg.canonicalOutputDir, err = filepath.EvalSymlinks(absOut)
	if err != nil {
		return mcpConfig{}, fmt.Errorf("canonical output path symlink: %w", err)
	}
...
func main() {
	...
	if _, err := scope.LoadDir(cfg.scopeDir); err != nil {
...
func handleScan(ctx context.Context, req mcp.CallToolRequest, cfg mcpConfig, registry *scanRegistry) (*mcp.CallToolResult, error) {
	...
	resolvedWordlist, err := resolvePathInAllowedDir(cfg.canonicalWordlistDir, wordlistName)
	if err != nil {
	...
	if err != nil {
		state.finish("", "", false)
		return mcp.NewToolResultError(fmt.Sprintf("failed to create scan output file: %v", err)), nil
	}
	defer f.Close()

	results, err := runScan(scanCtx, target, resolvedWordlist, cfg.maxThreads, cfg.maxResults, matchCodes, extensions, opts, state, f)
	if err != nil {
	...
	payload := buildScanOutput(target, scanID, startedAt, results, cfg.maxResults, outputName, nil)
...
type RecommendedParams struct {
	...
	SmartAPI   bool   `json:"smart_api"`
}

type BuildScanResponse struct {
	RecommendedParams RecommendedParams `json:"recommended_params"`
...
func handleBuildScan(ctx context.Context, req mcp.CallToolRequest, cfg mcpConfig) (*mcp.CallToolResult, error) {
	...
	case contains(descLower, "laravel", "php"):
		wordlist = bestWordlist(cfg.canonicalWordlistDir, []string{"php-common.txt", "common.txt"})
	...
	case contains(descLower, "django", "python", "flask"):
		wordlist = bestWordlist(cfg.canonicalWordlistDir, []string{"common.txt"})
	...
	case contains(descLower, "wordpress", "wp"):
		wordlist = bestWordlist(cfg.canonicalWordlistDir, []string{"wordpress.txt", "common.txt"})
	...
	case contains(descLower, "api", "rest", "graphql"):
		wordlist = bestWordlist(cfg.canonicalWordlistDir, []string{"api-endpoints.txt", "common.txt"})
		smartAPI = true
