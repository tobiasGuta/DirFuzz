...
- **Dynamic Role Rotation**: Implemented a rotating UI feature in the TUI list view that cycles through AuthMatrix identities every 5 seconds. It dynamically displays the varying HTTP status codes for each tested role directly on the single result line, along with a stylish Dracula Purple `[Role: <name>]` tag.
