...
func (p *ConnPool) Get(key string) net.Conn {
	...
	for len(conns) > 0 {
		...
		// Connection is still fresh enough; return it.
		// The client's write retry logic will handle it if the connection was
		// closed by the server while sitting in the idle pool.
		return pc.conn
	}
