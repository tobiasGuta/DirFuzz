...
func (e *Engine) executeAuthMatrixRequests(
	...
	for i, role := range roles {
		wg.Add(1)
		go func(idx int, r authMatrixRole) {
			defer wg.Done()
			roleHeaders := mergeAuthHeaders(baseHeaders, r.headers)
			...
			results[idx] = authMatrixRoleResponse{
				role:       r.role,
				level:      r.level,
				rawRequest: rawReq,
				resp:       resp,
...
func renderHeaderBlock(headers map[string]string) string {
	...
	for _, k := range keys {
		...
		safeV := sanitizeHeaderToken(headers[k])
