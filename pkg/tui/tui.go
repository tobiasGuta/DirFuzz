...
func (m *Model) renderPerformanceDashboard() string {
	...
	m.Engine.Config.RUnlock()
	if maxWorkers > 0 {
		currentUtil = float64(m.activeWorkers) / float64(maxWorkers) * 100
	}
...
type tickAuthCycleMsg time.Time

func tickAuthCycleCmd() tea.Cmd {
	return tea.Tick(5*time.Second, func(t time.Time) tea.Msg {
		return tickAuthCycleMsg(t)
	})
}

func (m Model) Init() tea.Cmd {
	return tea.Batch(tickCmd(), tickAuthCycleCmd(), m.listenForResults(), m.listenForLogEvents())
}
...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tickAuthCycleMsg:
		m.authCycleIdx++
		if m.state == StateList {
			m.renderListView()
		}
		return m, tickAuthCycleCmd()
	...
	case TickMsg:
		...
		utilization := 0.0
		if maxWorkers > 0 {
			utilization = float64(m.activeWorkers) / float64(maxWorkers)
		}
	...
	case ResultMsg:
		...
		if result.IsAutoFilter {
			...
			m.appendLog(formatResult(result, m.authCycleIdx), &result)
		}
		m.dashboardDirty = true
...
func (m *Model) renderListView() {
	...
	for visibleIdx := start; visibleIdx < end; visibleIdx++ {
		...
		if i < len(m.logLineHits) {
			...
			if lineHit != nil {
				line = formatResult(*lineHit, m.authCycleIdx)
			}
		}
...
func (m *Model) exportLogsToFile(path string) error {
	...
	if base != path || strings.Contains(base, "..") {
		return fmt.Errorf("invalid export path: must be a filename, not a path")
	}
	safePath := filepath.Join(".", base)
	...
	return os.WriteFile(safePath, []byte(sb.String()), 0o644)
}
...
func (m *Model) appendLog(text string, hit *engine.Result) {
	...
	if hit != nil {
		...
		if text == "" {
			text = formatResult(hitCopy, m.authCycleIdx)
		}
		hit = &hitCopy
	}
...
func formatResult(r engine.Result, authCycleIdx int) string {
	methodStr := r.Method
	if methodStr == "" {
		methodStr = "GET"
	}
	...
	extras := ""
	if statusCode == 403 && r.Forbidden403Type != "" {
